...
# RUN cmake -G "Ninja" -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=/root/install/upmem-llvm -DLLVM_ENABLE_PROJECTS="clang" /root/llvm-project/llvm 
RUN cmake -G "Unix Makefiles" -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=/root/install/upmem-llvm -DLLVM_ENABLE_PROJECTS="clang" /root/llvm-project/llvm 
RUN make -j4
RUN make install
